  ...
  <section>
    <div class="row">
      <div class="article-text col-md-8">
        <h2>
          {{ article.ArticleName
          }}<span v-if="article.TranslatorYearBirth"
            >, {{ article.TranslatorYearBirth }}–{{
              article.TranslatorYearDeath
            }}</span
          >
        </h2>

        <p class="redirected" v-if="isRedirected">
          (Omdirigerad från {{ redirectedName }})
        </p>
        ...
        <p class="mt-4">
          <nuxt-link
            ...
            :key="contrib.URLName"
            :to="'/medarbetare/' + contrib.URLName"
            rel="author"
            >{{ contrib.FirstName }} {{ contrib.LastName }}</nuxt-link
          >
        </p>
        ...
        <section class="awarded colorlinks" v-if="prizes.length">
          ...
          <ul>
            <li v-for="item in prizes">
              {{ item.Year }}
              <nuxt-link :to="'/artiklar/' + item.URLName">{{
                item.Prize
              }}</nuxt-link>
            </li>
          </ul>
      ...
      <div class="bibliography col" v-if="works.length">
        <header>
          ...
          <div class="detailed">
            <nuxt-link :to="'/listor/' + biblLink + '/' + actualId"
              >Gå till detaljerad bibliografi</nuxt-link
            >
          </div>
        ...
        <ul>
          <li v-for="item in connectionGroups">
            ...
            <h3 v-else-if="item.type == 3">
              Skrifter av {{ article.ArticleName }}
            </h3>
            ...
            <ul v-if="[2, 3, 4, 5].includes(item.type)">
              <li v-for="work in item.works">
                <nuxt-link class="work" :to="'/verk/' + work.id">{{
                  work.TitleSwedish
                }}</nuxt-link>
                <span v-if="work.Authors"> / {{ work.Authors }}</span>
              </li>
            </ul>
        ...
        <template v-if="links.length">
          ...
          <ul>
            <li v-for="link in links">
              <a
                v-if="link.ArticleLinkURL.match('^(http|https)')"
                :href="link.ArticleLinkURL"
                >{{ link.ArticleLinkTitle }}</a
              >
              <nuxt-link v-else :to="link.ArticleLinkURL">{{
                link.ArticleLinkTitle
              }}</nuxt-link>
            </li>
          </ul>
        </template>
    ...
    <div class="prizewinners row flex-column no-gutters" v-if="prizewinners">
      ...
      <ul>
        <li v-for="article in prizewinners">
          <nuxt-link
            v-if="article.ArticleName"
            :to="'/artiklar/' + encodeURIComponent(article.URLName)"
            >{{ article.Year }} {{ article.PrizeWinner }}</nuxt-link
          >

          <span v-if="!article.ArticleName"
            >{{ article.Year }} {{ article.PrizeWinner }}</span
          >
        </li>
      </ul>
    </div>
