...
export default {
    ...
    async asyncData({ params, error, route, from, $ua }) {
        ...
        if (
            from &&
            (from.matched[0].name == "listor-avoversattare-id" ||
                from.matched[0].name == "avoversattare-filter")
        ) {
        ...
        filterWorks: function(works) {
            ...
            return works.filter(work => {
                if (type == "original") {
                    ...
                    return (
                        lang == work.LanguageSourceName ||
                        (work.LanguageSourceName === null && lang == work.LanguageOriginalName)
                    )
                } else if (type == "till") {
